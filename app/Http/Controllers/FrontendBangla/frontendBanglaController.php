...
class frontendBanglaController extends Controller
{
    public function index(){
        ...
        $imagegallery_post_bangla = Post::where('add_image_gallery' , '1')->where('title_bn', '!=', '')->get();
        ...
        //     'Sports' => Post::where('category_id' ,'7')->where('stage', 'approved')->where('status' , '1')->where('title_bn', '!=', '')->limit(7)->get(),
        ...
        return view('FrontendBangla.home', compact( 'vedioGaleries' ,'posts' ,'categories','category_box', 'imagegallery_post_bangla','inter_categories','specific_news','main_posts'));
    }

    public function videoGallery($videoTitle)
     {
        $video = VideoGallery::where('video_title_bn', $videoTitle)->first();
        $videos = VideoGallery::all();
        return view('FrontendBangla.video_gallery.index' , compact('video' , 'videos'));
     }

     public function editorialPolicy($type)
     {
         $page_detail = PageTable::where('page_type' , $type )->first();
         return view('FrontendBangla.page.index' , compact('page_detail'));
     }
     ...
     public function profile()
     {
        ...
        return view ('FrontendBangla.profile.index',compact('user', 'district' , 'division' , 'subdistrict' , 'user_division_name' , 'user_distrit_name' , 'user_sub_distrit_name'));
     }

     public function covid_form()
     {
        $covid_posts = Post::where('category_id','14')->whereIn('post_type_id', ['1','3'])->where('published_date' , '<=' , date("Y/m/d"))->get();
        $data = null; 
        $country = CovidTacker::all('name');  
        // dd($country);
        $data = Http::get('https://coronavirus-19-api.herokuapp.com/countries/')->json();
        return view('bangla_covid_tracker', ['country'=>$country , 'data'=>$data, 'covid_posts'=>$covid_posts]);
    }
