                ...
                <div class="blog-page-container" id ="blog-post-by-categoryid" >
                    ...
                    <div class="single-news-wrapper">
                        ...
                        <div class="page-title">
                            <h1>{{$blog_post->blog_title_en}}</h1>
                        </div>
    
                        <div class="title-bottom">

                            <div class="post-author">
                                <span class="post-author-name">{{ $blog_post->user->name_en  }}</span> 
                                <span class="post-date">{{\Carbon\Carbon::parse($blog_post->published_date)->isoFormat('dddd, MMM Do YYYY')  }}</span>
                            </div>
                        ...
                        <div class="post-section">
                            ...
                            <div class="post-details">
                                <p><strong> {!! Str::limit( $blog_post->description_en ,50)!!}  </strong></p>
                        ...
                        <div class="post-metarials">
                            <div class="post-meta">
                                <span class="p-meta post-tag"><i class="fa fa-tag" aria-hidden="true" style="color:red" ></i>
                                    <a href="">{{ $blog_post->tag_en }}</a>
                                </span>
                                <span class="p-meta posy-comments"><i class="fa fa-comment" aria-hidden="true" style="color:red"></i>{{ $blog_post->blogcomments->where('approved' , '1')->count()}} Comments</span>
                                <span class="p-meta posy-comments"><i class="fa fa-eye" aria-hidden="true" style="color:red"></i><a href="">{{ $blog_post->view }} Hits</a></span>
                                <span class="p-meta print"><i class="fa fa-print" aria-hidden="true"></i><a href="{{ route('blog_postpdf.download', $blog_post->id) }}">Print this News</a></span>
                            </div>
                        </div>
                        ...
                        <div class="page-navigation">
                            <div class="prev-btn btn">
                                <a href="@isset($previous){{route('frontend.blog.postById' , [ 'slug_en' =>  $previous->slug_en] ) }} @endisset">
                                    <span><i class="fa fa-long-arrow-left" aria-hidden="true"></i></span>
                                    <strong>@isset($previous){{ Str::limit( $previous->blog_title_en , 100)}} @endisset </strong>
                                </a>
                            </div>
                            <div class="next-btn btn">
                                <a href="@isset($next){{route('frontend.blog.postById' , [ 'slug_en' =>  $next->slug_en] ) }} @endisset">
                                    <strong>@isset($next){{Str::limit( $next->blog_title_en , 100) }} @endisset</strong>
                    ...
                    <div class="related-news">
                        ...
                        <div class="title">
                            <h2>Blog Related Post</h2>
                        </div>
                        @foreach($blog_related_posts as $key => $blog_related_post)
                            @if($key <= 1)
                                <div class="related-news-box">
                                    <a href="{{route('frontend.blog.postById' , [ 'slug_en' =>  $blog_related_post->slug_en] ) }}">
                                        <img src="{{asset($blog_related_post->image)}}" alt="">
                                        <p class="news-title">{{$blog_related_post->blog_title_en}}</p>
                                    </a>
                            ...
                            <div class="related-news-list">
                                <ul>
                                    @if($key > 1)
                                        <li><a href="{{route('frontend.blog.postById' , [ 'slug_en' =>  $blog_related_post->slug_en] ) }}">{{$blog_related_post->blog_title_en}}</a></li>
                                    @endif
                                </ul>
                            </div>
                        @endforeach
                                ...
                                <ul class="newscategory-menu">
                                    ...
                                    @foreach($blog_categories as $blog_category)
                                        ...
                                        @if($blog_category->status==1)
                                            <li><a href="{{ route('frontend.blog.postBycategoryId' , $blog_category->id) }}" data-slug_bn="{{ $blog_category->blog_category_slug_bn }}" data-id="{{ $blog_category->id }}" class="blog-post-by-category" >{{$blog_category->blog_category_name_bn}}</a></li>
                                        @endif
