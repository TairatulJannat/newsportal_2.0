        ...
        <div class="topbar-nav d-flex h-50 justify-content-between">
            <div class="h-50 tobar-left-sec">
                <div class="tobar-left">
                    <ul class="topbar-menu">
                        <li><a class="topbar-menu-item" href="{{ route( 'frontend.home' )}}">Home</a></li>
                        {{-- <li><a class="topbar-menu-item" href="#">About</a></li>
                        <li><a class="topbar-menu-item" href="#">Terms</a></li> --}}
                        {{-- @if ($user !== null)
                        <li><a class="topbar-menu-item" href="{{ route('frontend.profile') }}">Profile</a></li>
                        @endif --}}
                        ...
                        <li>
                            {{-- <div class="usericon"> --}}
                                ...
                                @if ($user == null)
                                    <a href="" data-bs-toggle="modal" data-bs-target="#login_modal"><span class="login-icon"><i class="fa fa-user fauser" aria-hidden="true"></i> Login</span></a>
                                @else
                                    <div class="">
            ...
            <div class="">
                <div class="topbar-right d-flex">
                    ...
                    <div class="user">
                        @isset($user)
                            <a href="{{ route('frontend.profile') }}" class="user-name"><span><i class="fa fa-user fauser" aria-hidden="true"></i> @isset($user->name_en) {{ $user->name_en }} @endisset</span></a>
                        @endisset
                    </div>

                    <div class="login-btn" style="display: none">
                        @if ($user == null)
                            <a href="" data-bs-toggle="modal" data-bs-target="#login_modal" class="parent-btn"><span class="login-icon-2"><i class="fa fa-user fauser" aria-hidden="true"></i> Login</span></a>
                        @endif
                    </div>
...
<div class="topbar-brand">
    <div class="brand-middle">
        <a href="{{ route( 'frontend.home' )}}" class="brand"><img src="@isset($settings->fontend_header_logo){{ asset($settings->fontend_header_logo)}}@endisset" alt=""></a>
        <div class="date">
...
<div class="header-area">
    <div class="container">
        <nav class="headernav">
            <ul class="main-menu">
                @foreach ($categories_head_for_english as $category)
                    <li class="sub-menu-parent">
                        <a href="{{ route('frontend.post.categorytBypost', $category->category_slug_en ) }}" class="mainmenu-link">{{ $category->category_name_en }}
                        ...
                        <ul class="sub-menu">
                            ...
                            <li ><a  href="{{ route('frontend.post.subcategorytBypost', $subcategory->subcategory_slug_en ) }}">{{ $subcategory->subcategory_name_en  }}</a></li>
                            @endforeach
                        </ul>
                    </li>
                @endforeach
                <li > <a href="{{ route('covid.form') }}" class="mainmenu-link" >COVID-19</a></li>
                <li> <a href="{{ route('frontend.blog.index') }}" class="mainmenu-link" >Blog</a></li>
                <li> <a href="{{ route('frontend.archive.index') }}" class="mainmenu-link" >Archive</a></li>
            </ul>
            <div class="language">
                <ul class="main-menu">
                    <li class="sub-menu-parent">
                        <a href="{{ route('frontend.home.bangla') }}" class="mainmenu-link">বাংলা</a>
                        {{-- <ul class="sub-menu">
...
@php
    $feature_posts = DB::table('posts')->where('feature_news' , '1')->where('stage' , 'approved')->where('title_en', '!=', '')->where('status' , '1')->orderBy('created_at', 'desc')->limit(7)->get();
@endphp
        ...
        <div class="topcategory-area">
            <ul class="topcategory-slider">
                ...
                <li class="topsingle-category">
                    <a href="{{route('frontend.post.categorypostbyId' , [ 'slug_en' =>  $post->slug_en] ) }}">
                        <img src="{{ asset($post->image) }}" alt="" style="max-width: 80px; max-height: 70px;" />
                        <div class="topsingle-title">
                            <p>{{ Str::limit($post->title_en , 30) }}</p>
                        </div>
                    </a>
                </li>
                @endforeach
...
@include('frontend.layouts.partial.mobile_nav')

<!-- start breaking news -->

@php
    $breaking_news= App\Models\Post::where('breaking_news','1')->where('title_en' , '!=' , '')->get();
        ...
        <div class="ticker__viewport">
            <ul class="ticker__list" data-ticker="list">
                @foreach ($breaking_news as $post)
                    <li class="ticker__item" data-ticker="item"><a href="{{route('frontend.post.categorypostbyId' , ['slug_en' =>  $post->slug_en] ) }}">{{ ($post->title_en) }}</a></li>
                @endforeach
            </ul>
        </div>
    </div>
