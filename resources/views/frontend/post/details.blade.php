...
@section('meta')
    ...
    <meta name = "meta_tag" content="{{ $posts->tags_en }}">
    <meta name = "meta_title" content="{{ $posts->title_en }}">
    <meta name = "meta_author" content="{{ $posts->user->name_en }}">
    <meta name = "meta_description" content="{{  $posts->details_en  }}">
    @php
      $removetag =['+','-','*','/' , '?','&nbsp' , '!' , '%','=','<em>','</em>','<strong>','<p>','</p>','</strong>'] ;
      $posts_share_en = str_replace($removetag, "", $posts->details_en); 
    @endphp

  <meta property="og:url" content="{{Request::fullUrl()}}" />
  <meta property="og:type" content="website" />
  <meta property="og:title" content="{{ $posts->title_en }}" />
  <meta property="og:description" content="{{$posts_share_en}} | Published: {{ \Carbon\Carbon::parse($posts->published_date)->diffForHumans() }}" />
            ...
            <div class="blog-page-container">
                ...
                <div class="single-news-wrapper">
                    ...
                    <div class="page-title">
                        <h1>{{ $posts->title_en }}</h1>
                    </div>

                    <div class="title-bottom">
                        
                        <div class="post-author">
                            <span class="post-author-name">{{ $posts->user->name_en  }}</span>
                            <span class="post-date">{{\Carbon\Carbon::parse($posts->published_date)->isoFormat('dddd, MMM Do YYYY')  }}</span>
                        </div>
                        <div class="social d-flex">
                            <a href="{{ route('postpdf.download', $posts->id) }}"><i class="fa fa-print" aria-hidden="true"></i></a>
                    ...
                    <div class="post-section">
                        ...
                        <div class="post-img">
                            <a href="{{ asset($posts->image) }}" target="_blink">
                    ...
                    <div class="post-metarials">
                        <div class="post-meta">
                            ...
                            @php $split_tags_en = explode(",", $posts['tags_en']); @endphp
                            
                            @foreach ($split_tags_en as $split_tag)
                                <span class="p-meta post-tag"><i class="fa fa-tag" aria-hidden="true" style="color:red"></i>
                                    <a href="{{route('frontend.post.getpostbyseo' ,  ['tags_en' =>  $split_tag] )}}">{{ $split_tag}}</a> 
                                </span>
                            @endforeach
            
                            <span class="p-meta posy-comments"><p><i class="fa fa-comment" aria-hidden="true" style="color:red"></i>{{ $posts->comments->where('approved' , '1' )->count()}} Comments</p></span>
                            <span class="p-meta posy-comments"><p><i class="fa fa-eye" aria-hidden="true" style="color:red"></i>{{ $posts->view }} Hits</p></span>
                            ...
                            <span class="p-meta post-tag"><i class="fas fa-bookmark" style="color:red"></i></span>
                        </div>
                    </div>
                    {{-- ========== Next / Previous buttons ======== --}}
                    <div class="page-navigation">
                        <div class="prev-btn btn">
                            <a href="@isset($previous) {{route('frontend.post.categorypostbyId' , ['slug_en' =>  $previous->slug_en] ) }}@endisset">
                                <span><i class="fa fa-long-arrow-left" aria-hidden="true"></i></span>
                                <strong>@isset($previous){{  $previous->title_en }} @endisset </strong>
                            </a>
                        </div>
                        <div class="next-btn btn">
                            <a href="@isset($next) {{route('frontend.post.categorypostbyId' , ['slug_en' =>  $next->slug_en] ) }}@endisset ">
                                <strong> @isset($next) {{  $next->title_en }} @endisset </strong>
                    ...
                    @foreach($related_posts as $key => $related_post)
                        ...
                        @if($key <= 5)
                            <div class="related-news-box">
                                <a href="{{route('frontend.post.categorypostbyId' , ['slug_en' =>  $related_post->slug_en] ) }}">
                                <img src="{{ asset($related_post->image )}}" alt="">
                                    <p class="news-title">{{ $related_post->title_en }}</p>
                                </a>
                                ...
                                <div class="news-details">
                                    <p>{!! Str::limit( $related_post->details_en )!!}</p>
                                    <a href="{{route('frontend.post.categorypostbyId' , [ 'slug_en' =>  $related_post->slug_en] ) }}" class="read-btn">Read More <span><i class="fa fa-angle-right" aria-hidden="true"></i></span></a>
                                </div>
                            </div>
                        @endif
                                    ...
                                    <div class="comment-wrapper">
                                        <div class="panel panel-info">
                                                            ...
                                                            @if($message->user_id == $user->id && $message->approved == 0)
                                                                <li class="media">
                                                                    ...
                                                                    <div class="media-body m-body">
                                                                        ...
                                                                        <p style = "color:coral">your message is waitting for approved</p>
                                                                </li>
                                                            @endif
                                                        @endisset
                                                    @endforeach
                                            ...
                                            <form action="{{ route('frontend.comment.store') }}" method="post">
                                                ...
                                                @else
                                                    <button type="button" id ="post" name="submit" class="btn btn-success pull-left" >Post</button>
                                                @endif
                                            </form>
                                        </div>
                                        <br><br>
