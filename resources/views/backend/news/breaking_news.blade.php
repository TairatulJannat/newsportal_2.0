...
<div class="content">
    <div class="container-fluid">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header card-header-primary card-header-icon ">
                        ...
                        <h4 class="card-title">MANAGE BREAKING NEWS</h4>
                    </div>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    @if(Auth::user()->type == 'editor' || Auth::user()->type == 'admin' || Auth::user()->type == 'super_admin' )
                                        <th class="text-center" style="width:8%;">Breaking News</th>
                                                        ...
                                                        <td class="text-center">
                                                            @if($post->breaking_news == 1)
                                                                <div class="togglebutton">
                                                                    <label>
                                                                        <input type="checkbox" checked data-id="{{$post->id}}" onchange="breaking_news({{$post->id}})" name="breaking_news" class="breaking_news" value="{{$post->breaking_news }}">
                                                            ...
                                                            @else 
                                                                <div class="togglebutton">
                                                                    <label>
                                                                        <input type="checkbox"  data-id="{{$post->id}}"  onchange="breaking_news({{$post->id}})" name="breaking_news"  class="breaking_news" value="{{ $post->breaking_news }}">
                                                                        <span class="toggle"></span>
                                                                    </label>
                                                                </div>
                                                            @endif
                                                    ...
                                                    @if(Auth::user()->type == 'editor' || Auth::user()->type == 'admin' || Auth::user()->type == 'super_admin')
                                                        ...
                                                        <td class="text-center">
                                                            @if($post->status == 1)
                                                                <div class="togglebutton">
                                                                    <label>
                                                                        <input type="checkbox" checked id="{{$post->id}}" onchange="status(this.id)" name="status" class="status" value="{{$post->status }}">
                                                                        <span class="toggle"></span>
                                                                    </label>
                                                                </div>
                                                            @else 
                                                                <div class="togglebutton">
                                                                    <label>
                                                                        <input type="checkbox" id="{{$post->id}}"  onchange="status(this.id)" name="status"  class="status" value="{{ $post->status }}">
                                                                        <span class="toggle"></span>
                                                                    </label>
                                                                </div>
                                                            @endif
      ...
      <!-- end row -->
      {{-- Own pending news --}}
    @if(Auth::user()->type == 'sub_editor' || Auth::user()->type == 'admin' ||Auth::user()->type == 'editor')
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header card-header-primary card-header-icon ">
                        ...
                        <h4 class="card-title">MANAGE OWN PUBLISHED POST</h4>
                    </div>
                                ...
                                @foreach ($posts as $key =>$post)
                                    @if ($post->stage == 'approved')
                                        @if(Auth::user()->id == $post->user_id )
                                            <tr>
                                                <td class="text-left align-top">{{ ++$key }}</td>
                                                <td class="text-left align-top"><img width="100" height="100" src="{{ asset($post->image) }}" style="object-fit: cover; alt=""></td> 
                                                <td class="text-left align-top">{{ $post->title_bn }}</td>
                                                <td class="text-left align-top">{{ $post->title_en }}</td>
                                                <td  class="text-left align-top">{{ $post->published_date }}</td>
                                                <td  class="text-center">
