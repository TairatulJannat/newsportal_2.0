...
<div class="covid-info-wrapper container">
    ...
    <div class="covid-status d-flex justify-content-between">
        ...
        <div class="select-country w-25">
            <form action="{{ route('bangla.covid.tracker') }}" method="post">
                @csrf
                <div class="select" >
    ...
    <div class="covid-newses archive mt-5">
        <h1 class="page-title" style="border-bottom: 1px solid #ddd; padding-bottom: .8rem;">কোভিড-১৯ খবর</h1>
            ...
            @foreach ($covid_posts as $covid_post)
                <li>
                    <div class="archive-details">
                        <div class="date">
                            <p> {{\Carbon\Carbon::parse($covid_post->published_date)->locale('bn')->isoFormat('LLLL')  }} </p>
                        </div>
                        <div class="title">
                            <a href="{{route('FrontendBangla.post.categorypostbyId' , ['slug_bn' =>  $covid_post->slug_bn] ) }}">
                                <h2>{{ Str::limit($covid_post->title_bn, 60) }}</h2>
                            </a>
                            <p>{!! Str::limit($covid_post->details_bn , 270) !!}</p>
                            <a href="{{route('FrontendBangla.post.categorypostbyId' , ['slug_bn' =>  $covid_post->slug_bn] ) }}" class="read-btn">আরও পড়ুন <span><i class="fas fa-angle-right"></i></span></a>
                        </div>
                        <div class="img">
...
@push('js')
    <script>
        ...
        $('#status-btn').click(function(){
                ...
                // alert(id);
                $.ajax({
                    url: "{{ route('bangla.covid.tracker') }}",
                    dataType: 'html',
                    method: 'GET',
                    data: {
                        id: id,
